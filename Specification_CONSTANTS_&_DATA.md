...
| `f.` Key | 表示 | 説明 |
|--------|------|------|
| `f.targeting` | ターゲッティング | 攻撃対象の選択ルール。遠距離フェーズ/近距離フェーズは/n隊列1:16,隊列2:8,隊列3:4,隊列4:2、隊列5:1,隊列6:1 /n魔法フェーズは隊列に依存せずに抽選される。/n選択対象はBOX抽選方式であり、その抽選内容は戦闘を超えて引き継がれる。BOXが空になるとまた新規に補充される。そのため、隊列5に物理攻撃を3回以上狙われる可能性は、敵が32回攻撃では決して発生しない。|
| `f.damage_calculation` | ダメージ計算 | ダメージは`攻撃力-防御力(貫通減算) × 各種倍率`で計算される。各種倍率とは属性倍率、耐性倍率、共鳴、怒り、勢い、パーティ補正などである。 |
| `f.row-based_modifier` | 隊列補正 | 隊列に応じた補正。主にターゲティング重み・命中減衰係数・防御判定時の参照対象を定義する。敵側は固定値扱い、プレイヤー側のみ行補正を受ける。 |
| `f.hit_detection` | 命中減衰 | 命中判定。用い、多段の後続ヒットほど命中率が減衰する。行動単位で計算され、通常攻撃・連撃・反撃系で減衰は引き継がれない。 |
...
| `f.rarity_scaling`| レアリティの性能向上 | レアリティ段階に応じた基礎性能補正。 |
