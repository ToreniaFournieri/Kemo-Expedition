...
| `f.` Key | 表示 | 説明 |
|--------|------|------|
| `f.targeting` | ターゲッティング | 攻撃対象の選択ルール。遠距離フェーズ/近距離フェーズは`physical_threat_weight_bag`、魔法フェーズは`magical_threat_weight_bag`から抽選する。物理脅威チケットは行1:16、行2:8、行3:4、行4:2、行5:1、行6:1で、行番号に対応するユニットを対象とする。 |
| `f.damage_calculation` | ダメージ計算 | 与ダメージ算出式。`max(1, (攻撃-防御貫通後値) × 各種倍率)`で処理し、属性倍率、耐性倍率、共鳴、怒り、勢い、パーティ補正を掛け合わせる。 |
| `f.row-based_modifier` | 隊列補正 | 隊列(行)に応じた補正。主にターゲティング重み・命中減衰係数・防御判定時の参照対象を定義する。敵側は固定値扱い、プレイヤー側のみ行補正を受ける。 |
| `f.hit_detection` | 命中減衰 | 命中判定。`baseChance × (decay^(Nth_hit-1))`を用い、多段の後続ヒットほど命中率が減衰する。`Nth_hit`は連撃・反撃系列ごとに独立してリセットされる。 |
| `f.counter` | 反撃 | 近距離フェーズで被弾後、条件を満たせば即時反撃する処理。`null-counter`で無効化可能。反撃回数は`a.counter`段階に応じて`f.NoA`を倍率変換する。 |
| `f.re-counter` | 再反撃 | 反撃に対する再反撃処理。`a.re-counter`保有時に即時発動し、`null-counter`の無効化判定を受ける。発動後も通常の命中判定・ダメージ計算を行う。 |
| `f.re-attack` | 連撃 | 攻撃後に追加攻撃を行う処理。`a.re-attack`条件を満たす場合、同一対象へ追撃し、各ヒットは通常どおり命中減衰とダメージ計算を適用する。 |
| `f.magical-counter` | 魔法反撃 | 魔法攻撃トリガー専用の反撃処理。`a.magical-counter`段階に応じて`f.NoA`倍率を設定し、即時に魔法攻撃として解決する。 |
| `f.covering-fire` | 援護射撃 | 味方行動に連動して発生する追撃処理。`a.covering-fire`保有かつ遠距離攻撃可能なユニットが同一対象へ即時射撃する。 |
| `f.reward` | 報酬計算 | 戦闘結果に応じて経験値・ゴールド・ドロップ・追加抽選の有無を算出する。討伐条件、敵ランク、神効果、レア抽選処理を反映する。 |
| `f.rarity_scaling`| レアリティの性能向上 | レアリティ段階に応じた基礎性能補正。ステータス初期値・成長率・強化効率などのスケーリング係数を定義する。 |
| `f.super_rare_scaling`| 超レアの性能向上 | 超レア専用の上位スケーリング。通常のレアリティ補正に追加で適用される特別係数・追加抽選枠・性能上限緩和を定義する。 |
...
| `God of Fortification` | 防備の神 | 効果:全員に物理防御2/3倍。休憩時間1.5-α倍 | 平和を望むならば、戦に備えよ。 |
