...
| `a.iaigiri`3 | 居合斬り3 | 物理ダメージをx2.5倍する(攻撃回数は半減しない) |
...
| `a.momentum` | 気勢 | 物理/魔法攻撃倍率1.5倍(受けたダメージ1%につき1%減,最小0.5倍) |
| `a.cunning` | 狡猾 | 自動売却額が1.2倍 |
| `a.bulwark` | 壁 | 真後ろの味方への遠距離攻撃を肩代わりする |
| `a.cyborgization` | サイボーグ化 | 命中+30(`c.accuracy+0.030`)、回避-20(`c.evasion-0.020`) |
| `a.covering-fire` | 援護射撃 | 味方近接攻撃の命中が1回のみなら遠距離射撃(攻撃回数半減) |
| `a.peddler` | 行商 | 移動時間が2/3になる |
| `a.composure` | 平静 | 命中率+10%加算 |
| `a.magical-counter` | 魔法反撃 | 魔法には魔法で反撃する(攻撃回数半減) |
| `a.focus` | 集中 | 命中ボーナスの効果が1.2倍になる |
| `a.prophecy` | 予言 | 報酬抽選内容が見えるようになる |
| `a.stealth` | 隠れ蓑 | HP24%未満の時、自身へのダメージをすべて回避する|
