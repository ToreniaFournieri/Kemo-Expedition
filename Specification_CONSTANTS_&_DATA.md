...
| `d.accuracy-v` | 命中+v | 命中の減衰率に加算。 命中の値が高いほど、複数の攻撃回数の際の命中回数が上振れする(減衰x0.90で20回攻撃では命中回数平均8.8回,命中+20では減衰x0.94で20回攻撃では命中回数平均10.5回) |
| `d.evasion-v` | 回避+v | 敵の命中の減衰率を減算。回避の値が高いほど、敵の複数攻撃回数の際の被弾回数が減少する(減衰x0.90で20回攻撃では命中回数平均8.8回,回避+20では減衰x0.88で20回攻撃では命中回数平均7.5回) |
