...
| `a.life-drain`3 | 吸血1 | 相手に与えたダメージの3/10を回復 |
...
| `a.death-touch`1 | 接死1 | 近接攻撃の命中毎に1/128の確率で即死 |
| `a.flying`1 | 飛行1 | 相手の近接攻撃回数が1/4になる |
| `a.free`1 | 逃走1 | 近接1タイミングで発動。戦闘から逃げる(戦闘は引分になる) |
| `a.frostbite`1 | 凍傷1 | 攻撃が1回以上命中した相手に対して、次フェーズ以降の相手の行動順番に-1を加えて遅らせる |
...
| `a.bind`1 | 拘束1 | 近接攻撃の命中毎に1/32の確率で相手の行動を封じる |
| `a.regeneration`3 | 再生3 | 近接9(開始)タイミングで発動。この戦闘で失ったHPの20%を回復する。近接フェーズ前までにHPが0となった場合には発動しない |
| `a.burn`1 | 火傷1 | 相手の近接攻撃の命中毎に、相手に与ダメージの1%を与える |
| `a.soul-reap`1 | 魂奪1 | 魔法0(終了)タイミングで発動。相手のHPが10％未満であった場合、相手は即死する。回避も復活もできない |
| `a.mutual-magic-amplify`1 | 魔法増幅1 | 双方魔法ダメージ1.3倍 |
| `a.mutual-magic-restraint`1 | 魔法抑制1 | 双方魔法ダメージ0.8倍 |
| `a.mutual-physical-amplify`2 | 物理増幅2 | 双方物理ダメージ1.4倍 |
| `a.mutual-physical-restraint`1 | 物理抑制1 | 双方物理ダメージ0.8倍 |
...
| `a.reanimate`3 | 即時蘇生3 | 自身のHPが0となったタイミングで発動。戦闘中1回だけHP30%で復活する |
| `a.auriferous`1 | 含金1 | 自身が受ける攻撃回数が多いほど、自身がドロップするアイテム確率が上がる(10回毎に+1回抽選確率上昇) |
| `a.magic-seal`1 | 魔封1 | [効] 魔法フェーズの最初の魔法を無力化する(相手だけでなく自身や味方にもこの制約を受ける) |
| `a.ambush`1 | 待ち伏せ1 | この戦闘中まだ相手が行動していなかった場合、与ダメージ1.3倍 |
| `a.mimic`1 | 模倣1 | [効] 相手のアビリティ1つを模倣する |
