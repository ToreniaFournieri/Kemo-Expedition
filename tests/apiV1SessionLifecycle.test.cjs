...
test('signUp/logIn/logOut own account creation, catch-up staging, durable commit, and player-save hand-back outside React', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-session-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1SessionLifecycle.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
