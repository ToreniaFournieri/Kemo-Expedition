...
test('applyApiV1Commit is transport-neutral and preserves the extracted gameplay-mutation behavior', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-commit-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1CommitOperations.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
