...
test('a catch-up Cycle costs each state its own Steps and modifiers', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-party-cycle-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/partyCycleDuration.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
