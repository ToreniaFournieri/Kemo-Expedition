...
test('compact progress items keep their printed output', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-compact-progress-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/compactProgressItems.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
      'import.meta.env': '{}',
  ...
  const result = spawnSync(process.execPath, [output], { encoding: 'utf8', env: process.env });
