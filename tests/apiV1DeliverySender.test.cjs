...
test('delivery sender: content capture, claim/send/settle/complete orchestration, and retry semantics', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-delivery-sender-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1DeliverySender.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
