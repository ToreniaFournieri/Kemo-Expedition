...
test('compact and full Expedition simulations use exact isolated run counts', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-read-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1ReadModels.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
