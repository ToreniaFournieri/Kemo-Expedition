...
test('runtime START, terrain, confusion, and END deity paths cross the C++ boundary', () => {
  assert.match(battleSource, /selectTimedIndex\(/);
  assert.match(battleSource, /resolveTimedConfusion\(/);
  assert.match(battleSource, /resolveTimedTerrainEffect\(/);
  assert.match(battleSource, /isTimedTriggerSlot\(/);
  assert.match(hookSource, /resolvePeriodicDeityHpEffectKernel\(/);
});
