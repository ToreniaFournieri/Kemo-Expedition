...
test('Application API authority owns durable commit, rollback, and replay semantics outside React', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-authority-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1Authority.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm', sourcemap: 'inline',
    define: {
  ...
  const result = spawnSync(process.execPath, ['--enable-source-maps', output], { encoding: 'utf8' });
