...
test('backup export, import, and reset: unit, authority-level revision/fencing/delivery_in_flight, and transport-neutral coverage', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-backup-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1Backup.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
