...
test('HTTP-shaped and trusted in-process adapters share one Application API handler set with identical results', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-application-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1ApplicationApi.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
...
test('HomeScreen supplies runtime ports only and no longer owns the API dispatcher or gameplay/save logic for it', () => {
  const source = fs.readFileSync(path.resolve('src/components/HomeScreen.tsx'), 'utf8');
  for (const forbidden of ['processApiV1Request', 'executeCommit(', 'buildApiV1ReadData', 'logInApiAccount', 'signUpApiAccount', "'fundamental/", 'stageApiV1ElapsedProgression', 'applyApiV1Commit']) {
