import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
import { resolve } from 'node:path';
import test from 'node:test';
import { getDungeonById } from '../../src/data/dungeons.ts';
import { ENEMIES } from '../../src/data/enemies.ts';
import { getApproxAfkCycleDurationMs } from '../../src/game/afkScheduler.ts';
import {
  beginBattleKernelMeasurement,
  endBattleKernelMeasurement,
} from '../../src/game/battleKernel.ts';
import { executeBattle } from '../../src/game/battle.ts';
import { getEncounterEnemyWithScaling } from '../../src/game/enemyScaling.ts';
import { hydrateGameState } from '../../src/game/saveCodec.ts';
import { decodePersistedState } from '../../src/game/storageCompression.ts';
import { simulateAfkPartyChunkForWorker } from '../../src/hooks/useGameState.ts';
import { setLanguage } from '../../src/i18n/index.ts';
import type { GameState } from '../../src/types.ts';
import { createSeededRandom } from './battleGoldenHarness.ts';

const SAMPLE_SAVE_PATH = resolve(
  process.cwd(),
  'sample_savedata/Exp8,7,6,5,4,3_set_for_test_v0.9.3_dev_20260820.kemoz',
);
const WARMUP_COUNT = 10;
const SAMPLE_COUNT = 60;
const AFK_CYCLE_DURATION_SCALE = 0.05;

function loadState(): GameState {
  const envelope = JSON.parse(readFileSync(SAMPLE_SAVE_PATH, 'utf8')) as { saveDataCompressed: string };
  return hydrateGameState(
    JSON.parse(decodePersistedState(envelope.saveDataCompressed)) as GameState,
  );
}

function percentile(values: readonly number[], ratio: number): number {
  const ordered = [...values].sort((left, right) => left - right);
  return ordered[Math.min(ordered.length - 1, Math.floor((ordered.length - 1) * ratio))] ?? 0;
}

function withRandom<T>(seed: number, operation: () => T): { result: T; draws: number } {
  const previous = Math.random;
  const seeded = createSeededRandom(seed);
  let draws = 0;
  Math.random = () => {
    draws += 1;
    return seeded();
  };
  try {
    return { result: operation(), draws };
  } finally {
    Math.random = previous;
  }
}

function createBattleFixture(state: GameState) {
  const party = state.parties[0];
  if (!party) throw new Error('Performance save is missing party 1');
  const dungeon = getDungeonById(party.selectedDungeonId);
  const floor = dungeon?.floors.at(-1);
  const bossId = floor?.rooms.find((room) => room.type === 'battle_Boss')?.bossId ?? dungeon?.bossId;
  const baseEnemy = ENEMIES.find((enemy) => enemy.id === bossId);
  if (!dungeon || !floor || !baseEnemy) throw new Error('Performance save is missing its selected boss encounter');
  return {
    party,
    enemy: getEncounterEnemyWithScaling(baseEnemy, dungeon, floor.floorNumber, 'battle_Boss'),
    terrainEffect: floor.terrainEffect,
  };
}

test('production battle baseline reports latency, random, event, and Wasm-boundary metrics', () => {
  setLanguage('ja');
  const state = loadState();
  const fixture = createBattleFixture(state);
  const run = (iteration: number) => withRandom(0x8e710001 + iteration, () => executeBattle(
    structuredClone(fixture.party),
    structuredClone(fixture.enemy),
    structuredClone(state.bags),
    fixture.party.currentHp,
    { terrainEffect: fixture.terrainEffect },
  ));

  for (let index = 0; index < WARMUP_COUNT; index += 1) run(index);
  const durationsMs: number[] = [];
  const randomDraws: number[] = [];
  const eventCounts: number[] = [];
  const boundaryCalls: number[] = [];
  for (let index = 0; index < SAMPLE_COUNT; index += 1) {
    beginBattleKernelMeasurement();
    const startedAt = performance.now();
    const measured = run(index + WARMUP_COUNT);
    durationsMs.push(performance.now() - startedAt);
    boundaryCalls.push(endBattleKernelMeasurement());
    randomDraws.push(measured.draws);
    eventCounts.push(measured.result.log.length);
  }

  const report = {
    engine: 'typescript-coordinator-cpp-primitives',
    samples: SAMPLE_COUNT,
    medianBattleMs: percentile(durationsMs, 0.5),
    p95BattleMs: percentile(durationsMs, 0.95),
    medianRandomDraws: percentile(randomDraws, 0.5),
    medianEventCount: percentile(eventCounts, 0.5),
    medianWasmBoundaryCalls: percentile(boundaryCalls, 0.5),
  };
  console.info('BATTLE_CPP_BASELINE', JSON.stringify(report));
  assert.ok(report.medianBattleMs > 0);
  assert.ok(report.medianWasmBoundaryCalls > 1);
});

test('production save-backed AFK baseline reports worker CPU', () => {
  const state = loadState();
  const durationsMs: number[] = [];
  let totalBoundaryCalls = 0;
  for (const [partyIndex, party] of state.parties.entries()) {
    beginBattleKernelMeasurement();
    const startedAt = performance.now();
    withRandom(0xaf000000 + partyIndex, () => simulateAfkPartyChunkForWorker(state, {
      partyIndex,
      cycleDurationMs: getApproxAfkCycleDurationMs(party, AFK_CYCLE_DURATION_SCALE),
      simulatedCompletedAt: Date.UTC(2026, 7, 20),
      cycleDurationScale: AFK_CYCLE_DURATION_SCALE,
      gameMode: 'm.kemo',
    }));
    durationsMs.push(performance.now() - startedAt);
    totalBoundaryCalls += endBattleKernelMeasurement();
  }
  const report = {
    engine: 'typescript-coordinator-cpp-primitives',
    parties: state.parties.length,
    totalWorkerCpuMs: durationsMs.reduce((total, duration) => total + duration, 0),
    projectedParallelWorkerMs: Math.max(...durationsMs),
    p95WorkerMs: percentile(durationsMs, 0.95),
    wasmBoundaryCalls: totalBoundaryCalls,
  };
  console.info('BATTLE_CPP_AFK_BASELINE', JSON.stringify(report));
  assert.equal(report.parties, 6);
  assert.ok(report.totalWorkerCpuMs > 0);
});
