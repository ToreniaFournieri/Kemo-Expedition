...
test('the Party projections and equipment commands work on a real, heavy save', async t => {
  const sample = path.resolve('sample_savedata/Exp8,7,6,5,4,3_set_for_test_v0.9.3_dev_20260820.kemoz');
  if (!fs.existsSync(sample)) return t.skip('the sample save is not present');
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-sample-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1SampleSave.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
