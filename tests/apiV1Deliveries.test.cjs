...
test('delivery jobs follow the specified lifecycle, complete once, and are cancelled or refused around save replacement outside React', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-deliveries-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1Deliveries.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
