...
test('Application API popup events are semantic, grouped, sequenced, and retained', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-popup-events-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1PopupEvents.profile.ts')], outfile: output, bundle: true,
