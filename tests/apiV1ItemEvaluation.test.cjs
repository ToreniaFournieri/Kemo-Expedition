...
test('equipmentEvaluation evaluates items for a character without changing the game', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-eval-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1ItemEvaluation.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
      'import.meta.env': '{}',
