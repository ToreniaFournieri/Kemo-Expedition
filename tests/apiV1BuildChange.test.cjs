...
test('character build changes validate, confirm equipment loss, and report auto-equipment changes', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bokemo-api-v1-build-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const output = path.join(directory, 'profile.mjs');
  await build({
    entryPoints: [path.resolve('tests/support/apiV1BuildChange.profile.ts')], outfile: output, bundle: true, platform: 'node', format: 'esm',
    define: {
