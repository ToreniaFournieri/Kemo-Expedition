...
| return | Turned_Back | name は連続攻略の積み重ねがまだ足りないと悟り、次の挑戦へ向けて帰路についた |
| return | Turned_Back | name は進行条件を満たせず、地図を閉じて拠点への道を選んだ |
| return | Turned_Back | name は門番の刻印に拒まれ、静かに撤収を指示している |
| return | Turned_Back | name は封印解除まであと一歩ずつだと確認し、隊列を整えて戻っている |
| return | Turned_Back | name は探索継続を断念し、次回に備えて情報を持ち帰っている |
| return | Turned_Back | name は条件未達の報せを受け、焦らず帰還して準備を立て直す |
| return | Turned_Back | name は連続攻略の記録を刻み、次の挑戦に備えて帰還判断を下した |
| return | Turned_Back | name は今回の攻略成功を足掛かりに、封印解除を目指して戻っている |
