...
// SpecRef: 2.3.1.2 | Multiplier and Functions | getUnlockedRaceAbilitiesFromBonuses
...
// SpecRef: 2.3.1.2 | Multiplier and Functions | computeCharacterStats
...
function getAbilityDescription(id: AbilityId, level: number): string {
  const descriptions: Record<AbilityId, (level: number) => string> = {
    ...
    howl: () => '遠距離2タイミングで発動。相手の次の攻撃回数4/5',
    predator_sense: () => '近接9(開始)タイミングで発動。相手のHPが50％未満の場合、命中+30',
    slow: () => '自身の行動順番に-1して遅くなる',
    corrode: () => '近接攻撃が1回以上命中した相手に対して、攻撃倍率x2/3',
    life_drain: () => '近接攻撃で相手に与えたダメージの3/10を回復',
    no_offense: () => '通常行動をしなくなる(反撃などは行う)',
    decompose: () => '近接2タイミングで発動。相手の物理防御力を2/3',
    swarm: () => '自身のHPが減ると攻撃倍率・防御倍率減(HP1%につき0.5%減少)',
    death_touch: () => '近接攻撃の命中回数 x 1/128の確率で即死',
    flying: () => '相手の近接攻撃時、その攻撃回数が1/4',
    ...
    ranged_confusion: () => '遠距離2タイミングで発動。1/8の確率で無作為に選んだ相手を敵対状態とする。遠距離0(終了)タイミングで敵対状態は解消する',
    self_destruct: () => '近接2タイミングで自爆し、相手に残ダメージの3/10を与える',
    oblivion: () => '無作為に選んだ相手のアビリティ1つをこの戦闘中無効にする',
    reanimate: () => '自身のHPが0となったタイミングで発動。HP30%で復活する(戦闘中1回のみ有効)',
    auriferous: () => '自身が受ける総攻撃回数10回毎に、自身がドロップするアイテム抽選確率を+1する',
    magic_seal: () => '魔法フェーズの最初の魔法を無力化する(相手だけでなく自身や味方にもこの制約を受ける)',
    ambush: () => '自身の通常行動時点でいずれの相手もまだこの戦闘中に行動していなかった場合、与ダメージ1.3倍',
