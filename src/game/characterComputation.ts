...
function getAbilityDescription(id: AbilityId, level: number): string {
  const descriptions: Record<AbilityId, (level: number) => string> = {
    ...
    howl: () => '相手の次の攻撃回数4/5',
    predator_sense: () => '相手のHPが50%未満の場合、命中+30',
    slow: () => '自身の行動順番に-1して遅くなる',
    corrode: () => '相手の攻撃倍率x2/3',
    life_drain: () => '与ダメージの7/10を回復',
    no_offense: () => '行動をしなくなる(反撃などは行う)',
    decompose: () => '相手の物理防御力を2/3',
    swarm: () => 'HPが減ると攻撃倍率・防御倍率減(HP1%につき0.5%減少)',
    death_touch: () => '近接攻撃1/6の確率で即死',
    flying: () => '相手の近接攻撃時、その攻撃回数が1/4',
    free: () => '近接1タイミングで戦闘から逃げる',
    frostbite: () => '相手の行動順番に-1を加えて遅らせる',
    ice_reflect: () => '氷属性のダメージ反射(3/10)',
    bind: () => '近接行動時、相手の行動を封じる',
    regeneration: () => '近接フェーズ開始時、この戦闘で失ったHPの20%を回復する',
    burn: () => '相手の攻撃回数分ダメージ受ける',
    fire_reflect: () => '火属性のダメージ反射(3/10)',
    soul_reap: () => '魔法フェーズの終わりに、相手のHPが10%未満であった場合、相手は即死する。回避も復活もできない',
    mutual_magic_amplify: () => '双方魔法ダメージ1.3倍',
    mutual_magic_restraint: () => '双方魔法ダメージ0.8倍',
    ranged_confusion: () => '1/6で遠距離2タイミングで遠距離フェーズのみ敵対状態',
    self_destruct: () => '近接2タイミングで自爆し、相手に残ダメージの3/10を与える',
    oblivion: () => '無作為に選んだ相手のアビリティ1つをこの戦闘中無効にする',
    reanimate: () => '戦闘中1回だけHP30%で復活する',
    auriferous: () => '被弾回数が多いほど自身がドロップするアイテム確率が上がる',
    ...
    upgrade_all_abilities: () => '自身の他のアビリティを1段階強化する(ペナルティ効果のアビリティは軽減される)',
  };
