...
export function executeBattle(
  ...
  const partyEffects = [
    createPartyEffectEntry('fighter', 'defender', () => '守護者', level => `(後列にいる味方への物理ダメージ × ${level >= 3 ? '1/2' : level === 2 ? '3/5' : '2/3'})`),
    createPartyEffectEntry('lord', 'command', () => '指揮', level => `(後列にいる味方の物理攻撃力 × ${level >= 3 ? '2.43' : level === 2 ? '1.35' : '1.2'})`),
    createPartyEffectEntry('sage', 'm_barrier', () => '魔法障壁', level => `(後列にいる味方への魔法ダメージ × ${level >= 3 ? '1/2' : level === 2 ? '3/5' : '2/3'})`),
  ];
