...
For a current visible shop entry, copy `observation.shop.lineupId` and its `stockEntryId` into `{"action":"buy-shop-item","partyId":1,"lineupId":"...","stockEntryId":"..."}`. Never guess or reuse these identifiers after refresh. Purchase results remain hidden until the command commits. Paid refresh is unavailable. For atomic equipment concentration, use `configuration.characters[].equipment: {"mode":"replace_all","itemIds":[...]}`. The server removes all submitted characters' equipment first, then assigns the highest-enhancement matching base item in character and item-list order. Use `autoEquipCharacterIds` for targeted Auto Equipment after all manual assignments. Character-level `remove-all-equipment` and `run-auto-equipment` remain available when listed in `_legalActions`.
...
3. Put build changes in `configuration.characters[].changes`. Use `equipment.itemIds` when exact base items and allocation priority matter. Include `autoEquip: true` for a whole-party recalculation or `autoEquipCharacterIds` for ordered targeted runs, and preview the result. A class change alone does not guarantee useful equipment or positive attack counts.
