...
Build 50 completes that inventory-operation slice. `inventoryMutation.ts` now owns add-one, remove-one, variant-status transition, sell-stack, and sell-all-owned transformations. Removal retains its last-copy `notown` representation and optional in-place mode; sales receive the Super Rare Prana resolver explicitly, preserve per-stack Prana precedence over Gold, and return detached currency totals plus sold-count diagnostics. The reducer retains selection and validation, equipment timing attribution, Party refresh behavior, Prana policy binding, and publication. Historical behavior is explicit: single-stack sale accepts any positive-count status, bulk sale selects only `owned`, item locks are not filtered at this inventory-record layer, and a no-eligible bulk request preserves state identity.

The next checkpoint should inventory the remaining direct record writes before further extraction. In particular, compare legacy array-to-record migration, debug-store grants, Jewel attach/detach inventory rewrites, and shop purchase installation against the shared add/remove primitives. Move only structurally identical transformations: legacy conversion belongs with save migration; debug grants may need a distinct no-auto-sell grant operation; Jewel workflows must keep attachment validation and equipment policy outside the record primitive. Add parity tests first for sold/not-owned revival, stack caps, `isNew`, attached Jewel keys, and mutation identity.
