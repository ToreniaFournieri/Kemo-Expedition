...
- Timed traversal is phase-aware: for every absolute timing it resolves `ranged`, `magical`, then `melee` triggers and that phase's normal actions. The source-defined positions are preserved: Howl at ranged 8; Unstable Core at ranged 4 and magical 0; Soul Reap at ranged 2; and Predator Sense, Free/Pursuit, Regeneration, Flying, Decompose, melee Confusion, and Self Destruct only in their melee positions.
- Conditional tape consumption follows the frozen coordinator: enemy Soul Reap selects and emits its random party target; Confusion draws a target before its success roll and draws nothing when no target exists; threat and Self Destruct fallback selections consume only their source-defined draws. Howl selects the last pending party effect and applies it to the next normal action only, never its Re-attack. Regeneration uses the aggregate party damage ledger for every party owner.
