...
3. Stage 8: all 6 `Type.Unknown` Resource rows closed (Builds 94-95); Stage 1's contract catalog is now complete. ~~Backup/import/reset test coverage.~~ (Done, Build 96 — also fixed a real `atob`/UTF-8 decode bug in `backup/import` that the new round-trip test surfaced.) Still open, roughly in order: Help document-serving; the delivery sender; then Settings UI migration.
