...
| 8 | Settings, files, delivery, Help, Resources | Small/high-value Settings panels done (Build 99); 5 large reference panels deferred |
...
3. Stage 8: all 6 `Type.Unknown` Resource rows closed (Builds 94-95); Stage 1's contract catalog is now complete. ~~Backup/import/reset test coverage.~~ (Done, Build 96 — also fixed a real `atob`/UTF-8 decode bug in `backup/import` that the new round-trip test surfaced.) ~~Help document-serving.~~ (Already done pre-Stage-8; this document's earlier claim was stale — found and corrected, Build 98.) ~~The delivery sender.~~ (Done, Build 98.) ~~Settings UI migration: small/high-value panels.~~ (Done, Build 99 — also fixed a `developerNewsNotification` localization bug and a real save-corruption risk in how import/reset would have persisted.) Only the five large deferred reference panels (Glossary, Item Compendium, Character Roster, Bestiary, Super Rare) and the Send Feedback reviewed exception remain, by user decision — Stage 8's high-value work is otherwise complete.
