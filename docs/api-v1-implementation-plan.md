...
Status as of v0.9.7 Build 83. `/api/v1` stays **test-only** (`allowEnable` is set only by the desktop `--api-v1-test` flag) until every public-cutover gate in Stage 9 passes.
...
- Still to do: Inventory and Vault reads (B3), then migrate the panes (B4). The Vault (debug store) has no API operation (Spec 8.4.3: no API), so it stays a reducer action.
