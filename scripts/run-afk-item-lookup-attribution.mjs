...
const temporaryDirectory = mkdtempSync(join(tmpdir(), 'bokemo-afk-item-lookup-'));
const outputPath = join(temporaryDirectory, 'profile.mjs');

try {
  buildSync({
    entryPoints: [resolve(process.cwd(), 'tests/support/afkItemLookupAttribution.profile.ts')],
    outfile: outputPath,
    bundle: true,
    platform: 'node',
    format: 'esm',
