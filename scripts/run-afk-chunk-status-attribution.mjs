...
const temporaryDirectory = mkdtempSync(join(tmpdir(), 'bokemo-afk-chunk-status-'));
const outputPath = join(temporaryDirectory, 'profile.mjs');

try {
  buildSync({
    entryPoints: [resolve(process.cwd(), 'tests/support/afkChunkStatusAttribution.profile.ts')],
    outfile: outputPath,
    bundle: true,
    platform: 'node',
    format: 'esm',
