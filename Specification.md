...
| `c.` Key | 表示 | 説明 |
|--------|------|------|
| `c.melee_attack+v` | 近攻+v% | 近接攻撃の最終ダメージを v% 乗算強化する(同一名ボーナスは重複無効) |
| `c.ranged_attack+v` | 遠攻+v% | 遠距離攻撃の最終ダメージを v% 乗算強化する(同一名ボーナスは重複無効) |
| `c.magical_attack+v` | 魔攻+v% | 魔法攻撃の最終ダメージを v% 乗算強化する(同一名ボーナスは重複無効) |
| `c.physical_defense+v` | 物防+v% | 物理防御の最終値を v% 乗算強化する(同一名ボーナスは重複無効) |
| `c.magical_defense+v` | 魔防+v% | 魔法防御の最終値を v% 乗算強化する(同一名ボーナスは重複無効) |
| `c.melee_NoA+v` | 近回数+v | 近接攻撃回数が v 回増える(同一名ボーナスは重複無効) |
| `c.ranged_NoA+v` | 遠回数+v | 遠距離攻撃回数が v 回増える(同一名ボーナスは重複無効) |
| `c.magical_NoA+v` | 魔回数+v | 魔法攻撃回数が v 回増える(同一名ボーナスは重複無効) |
| `c.accuracy+v` | 命中+v*1000 | 値が多いほどより多くの攻撃が命中するようになる(同一名ボーナスは重複無効) |
| `c.evasion+v` | 回避+v*1000 | 値が多いほどより多くの攻撃を回避するようになる(同一名ボーナスは重複無効) |
| `c.equip_slot+v` | 装備+v | 装備スロット数が v 増える(同一名ボーナスは重複無効) |
| `c.grit+v` | 根性+v | 近接攻撃の装備が出来るようになる。近接攻撃回数が　v 回増える(同一名ボーナスは重複無効) |
| `c.pursuit+v` | 追撃+v | 遠距離攻撃の装備が出来るようになる。遠距離攻撃回数が　v 回増える(同一名ボーナスは重複無効) |
| `c.caster+v` | 術者+v | 魔法攻撃の装備が出来るようになる。魔法攻撃回数が　v 回増える(同一名ボーナスは重複無効) |
| `c.penet_+v` | 貫通+v*100% | 敵の防御力を　v*100% 分無視する(同一名ボーナスは重複無効) |
...
| `c.` Key | 表示 | 説明 |
|---------|------|------|
| `c.armor_x1.x` | 鎧x1.x | 鎧カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.robe_x1.x` | 衣x1.x | 法衣カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.shield_x1.x` | 盾x1.x | 盾カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.sword_x1.x` | 剣x1.x | 剣カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.katana_x1.x` | 刀x1.x | 刀カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.gauntlet_x1.x` | 手x1.x | 籠手カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.arrow_x1.x` | 矢x1.x | 矢カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.bolt_x1.x` | ボx1.x | ボルトカテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.archery_x1.x` | 弓x1.x | 弓カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.wand_x1.x` | 杖x1.x | 杖カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.grimoire_x1.x` | 書x1.x | 魔導書カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
| `c.catalyst_x1.x` | 媒x1.x | 触媒カテゴリ装備の効果が 1.x 倍(同一名ボーナスは重複無効)  |
...
|Version  |Changes                                                                               |
|---------|--------------------------------------------------------------------------------------|
| **0.2.4** | Party State Machine update.  |
