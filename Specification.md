...
| `c.accuracy+v` | 命中+v*1000 | v*1000の分より多くの攻撃が命中するようになる |
| `c.evasion+v` | 回避+v*1000 | v*1000の分より多くの攻撃を回避するようになる |
| `c.equip_slot+v` | 装備+v | 装備スロット数が v 増える |
| `c.grit+v` | 根性+v | 近接攻撃の装備が出来るようになる。また、近接攻撃回数が　v 回増える |
| `c.pursuit+v` | 追撃+v | 遠距離攻撃の装備が出来るようになる。また、遠距離攻撃回数が　v 回増える |
| `c.caster+v` | 術者+v | 魔法攻撃の装備が出来るようになる。また、魔法攻撃回数が　v 回増える |
| `c.penet_+v` | 貫通+v*100% | 敵の防御貫通を　v*100% 分無視する |
...
| `c.` Key | 表示 | 説明 |
|---------|------|------|
| `c.armor_x1.x` | 鎧x1.x | 鎧カテゴリ装備に乗算補正 |
| `c.robe_x1.x` | 衣x1.x | 法衣カテゴリ装備の魔法防御系ステータスに乗算補正 |
| `c.shield_x1.x` | 盾x1.x | 盾カテゴリ装備に乗算補正 |
| `c.sword_x1.x` | 剣x1.x | 剣カテゴリ装備に乗算補正 |
| `c.katana_x1.x` | 刀x1.x | 刀カテゴリ装備に乗算補正 |
| `c.gauntlet_x1.x` | 手x1.x | 籠手カテゴリ装備に乗算補正 |
| `c.arrow_x1.x` | 矢x1.x | 矢カテゴリ装備に乗算補正 |
| `c.bolt_x1.x` | ボx1.x | ボルトカテゴリ装備に乗算補正 |
| `c.archery_x1.x` | 弓x1.x | 弓カテゴリ装備に乗算補正 |
| `c.wand_x1.x` | 杖x1.x | 杖カテゴリ装備に乗算補正 |
| `c.grimoire_x1.x` | 書x1.x | 魔導書カテゴリ装備に乗算補正 |
| `c.catalyst_x1.x` | 媒x1.x | 触媒カテゴリ装備に乗算補正 |

---

## 表示ルール（仕様用）

- `c.` ボーナスは装備・種族・クラス・性格・家系などから付与されるカテゴリ補正である。  
- 同名の `c.` ボーナスは「ユニークルール」に従い、同一キーは最も高い値のみ有効。  
- 異なる `c.` ボーナスは乗算または加算で合算される。  
- UI表示では、内部計算結果を小数1桁に丸めて表示する。  
