...
- The final engine uses battle-local splitmix64-seeded xoshiro256** version 1 initialized by one unsigned 64-bit seed per battle; equal protocol version, RNG version, battle input, and seed must produce an identical complete result. Each worker realm owns its own Wasm instance and no native RNG state is shared between battles or realms.
- The C++ RNG migration intentionally establishes the version 2 seeded replay contract without modifying the historical version 1 tape fixtures or frozen TypeScript reference. The v2 fixture is generated only through an explicit guarded command after frozen-reference, native-tape, and native-seeded equality succeeds.
- Completed seeded engine results identify protocol version, ABI version, RNG version, exactly 16 lowercase hexadecimal seed characters, and native random-draw count. Production callers acquire exactly one seed through one Web Crypto `getRandomValues` call over two `Uint32` words in low-word/high-word order; zero and maximum u64 are valid. Invalid explicit seeds and unsupported RNG versions fail before execution and are never wrapped or downgraded to tape mode.
