...
- After the complete non-random battle engine has passed tape-driven differential parity, protocol v2 transfers random-generation ownership to C++. The final engine uses a versioned deterministic PRNG initialized by one unsigned 64-bit seed per battle; equal protocol version, RNG version, battle input, and seed must produce an identical result.
...
- Protocol v1 input includes aggregate party/enemy HP, combatant records, grouped ability records, the ordered migration random tape, and physical and magical threat bags. Protocol v2 additionally includes deity, engine flags, the unsigned 64-bit seed, and RNG version. Output includes outcome, aggregate HP, enemy hits received, random-draw diagnostics, replay metadata, updated threat bags, and ordered language-neutral event records.
