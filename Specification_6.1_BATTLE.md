...
- During deterministic migration, JavaScript supplies a recorded random-value tape in canonical resolution order. A deterministic migration step must not add, remove, reorder, or prefetch draws, and `tests/fixtures/battleGolden.v1.json` must remain unchanged.
- Protocol v3 is the approved full-battle migration layout. TypeScript projects runtime game objects into complete static numerical combat profiles and supplies the frozen ordered random-value tape; C++ does not prefetch, reorder, or generate migration draws. Protocol v2 is insufficient for full-battle execution and must be rejected by the v3 entry point.
- A protocol v3 input contains aggregate party and enemy current/max HP; every combatant's current/max HP, current and original ranged/magical/melee NoA, attack and defense values, per-attack accuracy potency, accuracy/evasion bonuses, physical/magical penetration, fire/thunder/ice resistance, physical/magical offense and defense amplifiers, phase bonuses, deity-derived numerical bonuses, enemy attack amplifiers, and enemy magic style; mutable abilities and ownership; terrain, deity, engine flags, seed and RNG diagnostics; physical and magical threat bags; and the exact ordered random tape.
...
- The protocol v3 shadow candidate invokes `battle_protocol_execute` exactly once per battle and counts it as one measured TypeScript/WebAssembly boundary call. Production `executeBattle`, AFK, and API callers remain on the existing coordinator until every frozen golden case has complete result parity.
- After the complete tape-driven battle engine has passed differential parity, a later protocol revision may transfer random-generation ownership to C++. The final engine uses a versioned deterministic PRNG initialized by one unsigned 64-bit seed per battle; equal protocol version, RNG version, battle input, and seed must produce an identical result.
- The C++ RNG migration may intentionally change the prior random sequence and battle results. After the seeded implementation passes reproducibility, bounds, worker, API-sortie, and battle-invariant validation, `tests/fixtures/battleGolden.v1.json` is regenerated exactly once to establish the seeded-engine baseline.
- Seeded engine results and diagnostic records must identify the protocol version, RNG version, and replay seed. Production callers generate one seed using Web Crypto unless an explicit replay/test seed is supplied; `Math.random` is not the final battle seed source.
...
- Protocol v1 input includes aggregate party/enemy HP, combatant records, grouped ability records, the ordered migration random tape, and physical and magical threat bags. Protocol v2 additionally includes deity, engine flags, the unsigned 64-bit seed, and RNG version, but lacks the complete projected combat data required by full-battle execution. Protocol v3 replaces the v2 full-battle layout with the complete profile defined above. Output includes outcome, aggregate HP, enemy hits received, random-draw diagnostics, replay metadata, updated threat bags, and ordered language-neutral event records.
