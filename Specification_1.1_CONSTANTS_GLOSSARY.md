...
|----|----|----|----|
| `a.ambush`1 | 待ち伏せ1 | 自身の通常行動時点でいずれの相手もまだこの戦闘中に行動していなかった場合、与ダメージ1.3倍 | beforehand |　
| `a.ambush`2 | 待ち伏せ2 | 自身の通常行動時点でいずれの相手もまだこの戦闘中に行動していなかった場合、与ダメージ1.5倍 | beforehand |
| `a.ambush`3 | 待ち伏せ3 | 自身の通常行動時点でいずれの相手もまだこの戦闘中に行動していなかった場合、与ダメージ1.6倍 | beforehand |
| `a.ambush`4 | 待ち伏せ4 | 自身の通常行動時点でいずれの相手もまだこの戦闘中に行動していなかった場合、与ダメージ1.65倍 | beforehand |
| `a.ambush`5 | 待ち伏せ5 | 自身の通常行動時点でいずれの相手もまだこの戦闘中に行動していなかった場合、与ダメージ1.68倍 | beforehand |
| `a.shock`1 | 感電1 | 相手の最初の通常近接攻撃に対して発動。相手の近接攻撃が1回目ヒットした段階で攻撃をやめさせる | intercept |
...
| `a.reanimate`1 | 即時蘇生1 | 自身のHPが0となったタイミングで発動。HP20%で復活する(戦闘中1回のみ有効) | counter |
| `a.reanimate`2 | 即時蘇生2 | 自身のHPが0となったタイミングで発動。HP26%で復活する(戦闘中1回のみ有効) | counter |
| `a.reanimate`3 | 即時蘇生3 | 自身のHPが0となったタイミングで発動。HP31%で復活する(戦闘中1回のみ有効) | counter |
| `a.reanimate`4 | 即時蘇生4 | 自身のHPが0となったタイミングで発動。HP35%で復活する(戦闘中1回のみ有効) | counter |
| `a.reanimate`5 | 即時蘇生5 | 自身のHPが0となったタイミングで発動。HP38%で復活する(戦闘中1回のみ有効) | counter |
| `a.re-counter`1 | 再反撃1 | 敵から反撃に対して、反撃する(攻撃回数半減) | re-counter |
| `a.re-counter`2 | 再反撃2 | 敵から反撃に対して、反撃する(攻撃回数半減しない) | re-counter |
