...
- **Sub progress bar:**
| Normal Clear-Gate condition | 🚪0/8 1F-4解放 | 連続攻略成功 0/8 で 1F-4解放 |
...
- Normal Clear-Gate progress updates after the normal expedition outcome is finalized. A `Clear` or `Turned_Back` increments it, while `Draw_Retreat`, `Wounded_Retreat`, or `Defeat` displays the reset value `0/8`.
